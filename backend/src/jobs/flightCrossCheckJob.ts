...
export function initFlightCrossCheckJobs() {
    // Daily schedule: 00:05 Thailand Time (17:02 UTC)
...
export async function runScheduledCrossCheck() {
    ...
    // Adjusted to +1 day to match Thailand "today" (runs at 00:02 TH / 17:02 UTC)
    // const todayStr = new Date().toISOString().split('T')[0];
    // console.log(`[CRON] UTC Today: ${todayStr}`);
    const todayStr = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().split('T')[0];
    console.log(`[CRON] UTC Today (+1 Day for TH): ${todayStr}`);
