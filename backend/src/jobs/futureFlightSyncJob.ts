...
export function initFutureFlightSyncJobs() {
    ...
    futureSyncRepository.initializeTable()
        .then(async () => {
            ...
            // Reset to 2027-01-01 for this fix run (to recover from the previous skip)
            await futureSyncRepository.resetSyncDate('2027-01-01');
            
            // Trigger immediately once for testing (AFTER table is ready)
            runFutureFlightSync();
        })
