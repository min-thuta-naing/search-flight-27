...
export interface ContinentMappingDebug {
  input: {
  ...
  normalized: {
    ...
    countryNameFromIso?: string;
  };
  source: {
    csvContinent?: string;
    isGroupedFromCsv: boolean;
  };
  lookupPath: string;
  result: ContinentMeta;
}

export function debugGetContinentMapping(countryCode?: string | null, countryName?: string | null): ContinentMappingDebug {
  const normalizedCountryName = countryName ? normalizeCountryName(countryName) : undefined;
  let lookupPath = 'default-other';
  let derivedCountryNameFromIso: string | undefined;

  const byCountryName = lookupContinentByCountryName(countryName);
  if (byCountryName) {
    const csvContinent = lookupCsvContinentByCountryName(countryName) || undefined;
    lookupPath = `country-name (${normalizedCountryName})`;
    return {
      input: { countryCode, countryName },
      normalized: { countryNameNormalized: normalizedCountryName },
      source: {
        csvContinent,
        isGroupedFromCsv: Boolean(csvContinent) && normalizeText(byCountryName) !== normalizeText(csvContinent),
      },
      lookupPath,
      result: buildContinentMeta(byCountryName),
    };
  }

  const normalizedCode = (countryCode || '').trim().toUpperCase();
  if (normalizedCode) {
    derivedCountryNameFromIso = countryNameFromIsoCode(normalizedCode);
    if (derivedCountryNameFromIso) {
      const byCodeDerivedName = loadCountryToContinentMap().get(derivedCountryNameFromIso);
      if (byCodeDerivedName) {
        const csvContinent = lookupCsvContinentByCountryName(derivedCountryNameFromIso) || undefined;
        lookupPath = `iso-code (${normalizedCode}) → ${derivedCountryNameFromIso}`;
        return {
          input: { countryCode, countryName },
          normalized: { countryNameNormalized: normalizedCountryName, countryNameFromIso: derivedCountryNameFromIso },
          source: {
            csvContinent,
            isGroupedFromCsv: Boolean(csvContinent) && normalizeText(byCodeDerivedName) !== normalizeText(csvContinent),
          },
          lookupPath,
          result: buildContinentMeta(byCodeDerivedName),
        };
      }
    }
  }
  ...
  return {
    ...
    normalized: { countryNameNormalized: normalizedCountryName, countryNameFromIso: derivedCountryNameFromIso },
    source: {
      csvContinent: undefined,
      isGroupedFromCsv: false,
    },
    lookupPath,
    result: buildContinentMeta('Other'),
  };
}
